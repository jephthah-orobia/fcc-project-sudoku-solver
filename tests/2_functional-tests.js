...
suite('Functional Tests', () => {
    ...
    if (isLocal) {
        ...
        test('#13 Solve a puzzle with valid puzzle string', function (done) {
            chai.request(server)
                .post('/api/solve')
                .send({ puzzle: valid[0][0] })
                .end((err, res) => {
                    assert.equal(res.status, 200)
                    assert.isObject(res.body);
                    assert.hasAllKeys(res.body, solutionKey);
                    assert.isString(res.body.solution);
                    assert.equal(res.body.solution.length, 81);
                    assert.equal(res.body.solution, valid[0][1]);
                    done();
                });
        })

        test('#14 Solve a puzzle with missing puzzle string',
            function (done) {
                chai.request(server)
                    .post('/api/solve')
                    .send({ puzzle: '' })
        ...
        test('#15 Solve a puzzle with invalid characters', function (done) {
            chai.request(server)
                .post('/api/solve')
                .send({ puzzle: invalid_chars[0] })
                .end((err, res) => {
                    assert.equal(res.status, 200)
                    assert.isObject(res.body);
                    assert.hasAllKeys(res.body, errorKey);
        ...
        test('#16 Solve a puzzle with incorrect length',
            function (done) {
                chai.request(server)
                    .post('/api/solve')
                    .send({ puzzle: none81[0] })
                    .end((err, res) => {
                        assert.equal(res.status, 200)
                        assert.isObject(res.body);
                        assert.hasAllKeys(res.body, errorKey);
        ...
        test('#17 Solve a puzzle that cannot be solved', function (done) {
            chai.request(server)
                .post('/api/solve')
                .send({ puzzle: cannotBeSolved[0] })
                .end((err, res) => {
                    assert.equal(res.status, 200)
                    assert.isObject(res.body);
                    assert.hasAllKeys(res.body, errorKey);
        ...
        test('#18 Check a puzzle placement with all fields: A4 = 7 on valid.0.0', function (done) {
            chai.request(server)
                .post('/api/check')
                .send({
                    puzzle: valid[0][0],
        ...
        test('#19 Check a puzzle placement with single placement conflict: G9 = 2 on valid.1.0', function (done) {
            chai.request(server)
                .post('/api/check')
                .send({
                    puzzle: valid[1][0],
        ...
        test('#20 Check a puzzle placement with multiple placement conflicts: I3 = 9 on valid.4.0', function (done) {
            chai.request(server)
                .post('/api/check')
                .send({
                    puzzle: valid[4][0],
        ...
        test('#21 Check a puzzle placement with all placement conflicts: B6 = 1 on valid.4.0', function (done) {
            chai.request(server)
                .post('/api/check')
                .send({
                    puzzle: valid[4][0],
        ...
        test('#22 Check a puzzle placement with missing required fields', function (done) {
            chai.request(server)
                .post('/api/check')
                .send({
                    coordinate: 'I3',
        ...
        test('#23 Check a puzzle placement with invalid characters', function (done) {
            chai.request(server)
                .post('/api/check')
                .send({
                    puzzle: invalid_chars[0],
                    coordinate: 'A1',
                    value: '1'
                }).end((err, res) => {
                    assert.equal(res.status, 200);
        ...
        test('#24 Check a puzzle placement with incorrect length', function (done) {
            chai.request(server)
                .post('/api/check')
                .send({
                    puzzle: none81,
        ...
        test('#25 Check a puzzle placement with invalid placement coordinate',
            function (done) {
                chai.request(server)
                    .post('/api/check')
                    .send({
        ...
        test('#26 Check a puzzle placement with invalid placement value',
            function (done) {
                chai.request(server)
                    .post('/api/check')
                    .send({
